...
4. The setup panel: vanilla always keeps `<section class="setup-panel hidden">`
   in the DOM and toggles the class, while Preact omits the element entirely
   when closed. Handle this by seeding each scenario with the setup panel in
   the SAME visible state in both builds and normalizing the `hidden` class
   away in `extract`, so the panel's *contents* are still genuinely compared
   rather than skipped.
