...
- [ ] **Step 1b: Rebuild `processed` on every tick, not only on `start()`**

CRITICAL, found in Task 4's review. Vanilla reset `processedPickNos = new Set()`
in three handlers — CSV import, Reset Draft, and Reset Everything. The port
dropped all three call sites. If `processed` is only rebuilt inside `start()`,
then **Reset Draft during a live Sleeper draft leaves `processed` populated and
polling silently never re-imports the picks** — the board stays empty while the
draft runs, with no error.

Rebuild it from the store on each tick instead of trusting the accumulated set:

```js
      (picks) => {
        // Rebuild from the store every tick. Reset Draft / Reset Everything /
        // a CSV re-import all clear `picks`, and a stale accumulated set would
        // make polling silently skip every pick from then on.
...
This removes the need for the `processed` ref entirely — delete it. Add a test
or a browser check proving that Reset Draft mid-sync re-imports picks.
...
7. The setup panel's four uncontrolled inputs (`numTeams`, `rosterSpots`,
   `positionLimits`, `scoringNotes`) carry a `value` ATTRIBUTE that vanilla did
   not emit. Preact sets `defaultValue` as a DOM property, which reflects into
   the `value` attribute; vanilla assigned `el.value` (the property only) in
   `initSetupPanel()`. The displayed values are identical — verified in Task 4
   by extracting both panels: the ONLY differences across all 53 structural
   lines were these attributes. Compare inputs by their `value` PROPERTY, not
   the attribute. This one is unavoidable: `defaultValue` is what makes the
   keyed remount rewrite those fields on a Sleeper sync.
